...
var jtl = { //the "namespace" encapsulator
    ...
    hmsToMs: function (HH, MM, SS, ms) {
        ...
        if (!jtl.isUInt(HH + MM + SS + ms))
            return null; //exit if some of the imput params isn't integer value
    ...
    stopWatch: function () {
        ...
        this.splittime = function () {
            if (!isStarted)
                return null; //must start timer first
        ...
        this.stoptime = function () {
            ...
            isStarted = false;
            if (updateOn)
                clearInterval(updater); //stop the refresh timer if not needed anymore
        ...
        this.resettime = function () {
            if (isStarted)
                return false; //must stop timer first
    ...
    backWatch: function () {
        ...
        this.startHMS = function (H, M, S) {
            if (isStarted)
                return null; //already started or error
            else {
                isStarted = true;
                startTime = jtl.getTime();
                if (!jtl.isUInt(H + M + S))
                    return null; //bad input time
        ...
        this.stoptime = function () {
            if (!isStarted)
                return null; //exit if timer not started
